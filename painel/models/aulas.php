...
class Aulas extends model{
	...
	public function deleteAula($id){
		...
		$sql = "SELECT id_curso FROM aulas WHERE id = '$id' ";
            ...
            $this->db->query("DELETE FROM questionarios WHERE id = '$id'");
            $this->db->query("DELETE FROM videos WHERE id = '$id'");
            $this->db->query("DELETE FROM historico WHERE id = '$id'");
	...
	public function addAula($id_curso, $nome, $id_modulo, $tipo){
		...
		if($sql->rowCount()>0){
			...
			if($tipo == 'video'){
				...
				$this->db->query("INSERT INTO videos SET id_aula = '$id_aula' ");
			}
